...
- **FAT32** — MBR/BPB parsing (`mbr_boot_sector`, `fat32_ext_bs`); `fat32_is_boot_sector` validates the 0xAA55 signature and EBR sanity fields; `fat32_parse_boot_sector` extracts BPB/EBR fields and computes derived values into `fat32_bs_info`; `fat32_read_fat_table` copies one FAT sector into a flat `uint32_t` array (28-bit masked); 8.3 and LFN directory entry structs (`fat32_dir_entry`, `fat32_lfn_entry`) with `FAT32_ATTR_*`, `FAT32_DIRENT_*`, and `FAT32_LFN_*` defines; packed structs with aligned mirrors for safe field access on AArch64; dump functions for boot sector, EBR, dir, and LFN entries
