...
static int test_vfs_read(struct fs_node *node, uint8_t *buff, size_t count, size_t offset)
{
    printk("test_vfs_read(): node=0x%08x, buff=0x%08x, count=%d, offset=%d\r\n", node->name, buff, count, offset);
...
static int test_vfs_write(struct fs_node *node, uint8_t *buff, size_t count, size_t offset)
{
    printk("test_vfs_write(): node=0x%08x, buff=0x%08x, count=%d, offset=%d\r\n", node->name, buff, count, offset);

    char msg[50] = {0};
    strncpy(msg, (char *)buff, count);
...
static void test_vfs()
{
    ...
    char buffer[50] = {0};
    vfs_read("/test/file", (uint8_t *)&buffer, 50, 0);

    printk("writing \"/test/file\"...\r\n");
    char *msg = "wash my belly";
    vfs_write("/test/file", (uint8_t *)msg, strlen(msg), 0);
...
static int test_io_read(uint8_t *buff, size_t count, size_t offset, uint64_t drv_info)
{
    printk("test_io_read(): buff=0x%08x, count=%d, offset=%d, drv_info=0x%08x\r\n", buff, count, offset, drv_info);
...
static int test_io_write(uint8_t *buff, size_t count, size_t offset, uint64_t drv_info)
{
    printk("test_io_write(): buff=0x%08x, count=%d, offset=%d, drv_info=0x%08x\r\n", buff, count, offset, drv_info);
...
static void test_io_modules()
{
    ...
    char buffer[50] = {0};
    vfs_read("/dev/test", (uint8_t *)&buffer, 50, 0);

    printk("writing \"/dev/test\"...\r\n");
    char *msg = "wash my belly";
    vfs_write("/dev/test", (uint8_t *)msg, strlen(msg), 0);
