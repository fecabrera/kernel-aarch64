...
// Open-addressing hash table with linear probing; maps null-terminated string
// keys to uint64_t values. Keys are duplicated on insert and freed on remove
// or destroy. Grows automatically when the load factor reaches 70%.
...
/**
 * Inserts or updates the entry for key. The key is duplicated into a
 * heap-allocated buffer. Grows the backing array if the load factor reaches 70%.
 *
