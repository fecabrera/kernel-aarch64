 ...
 * @param offset:   byte offset into the device (does not need to be
 * sector-aligned)
 * @param slot:     virtio slot index (set by storage_init via
 * io_register_module)
 *
 * @return count on success, negative error code from virtio_mmio_read on
 * failure
 */
